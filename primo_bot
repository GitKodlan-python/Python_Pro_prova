...
async def on_message(message):
    ...
    if message.content.startswith('$Io sono triste'):
        await message.channel.send("Non mi interessa")
    if message.content.startswith('$saluto'):
        await message.channel.send("Ciao a tutti!")
    elif message.content.startswith('$arrivederci'):
        await message.channel.send("\U0001f642")
    else:
        await message.channel.send(message.content)

client.run("METTI IL TOKEN")
